...
class SnnewController extends AdminController {
    public function index(){
        if(is_login()){
	        ...
	        $snList = $this->lists('SnWujing',$map,'id asc');
	    	...
	    	foreach ($snList as &$v){	        	
	        	...
	        	switch ($v['status']){
	        		case 0: $v['status']='未使用';break;
	        		case 1: $v['status']='已使用';break;
	        	}
	...
	public function edit($id = 0){
        if(IS_POST){
            ...
            $info = array();
            /* 获取数据 */
            $info = M('SnWujing')->field(true)->find($id);
