...
function BU($url,$vars)
{
    ...
    if( (strpos($url,'user') !== false) && (strpos($url,'login') !== false) ){
        return '/account/login/'.$param;
    }elseif( (strpos($url,'user') !== false) && (strpos($url,'register') !== false) ){
        return '/account/register/'.$param;
    }elseif( (strpos($url,'user') !== false) && (strpos($url,'resetpwd') !== false) ){
        return '/account/resetpwd/'.$param;
    }else{
        return false;
    }
}
function U($url='',$vars='',$suffix=true,$domain=false) {
    ...
    if(isset($info['fragment'])) { // 解析锚点
        ...
        if(false !== strpos($anchor,'@')) { // 解析域名
            list($janchor,$host)    =   explode('@',$anchor, 2);
        }
