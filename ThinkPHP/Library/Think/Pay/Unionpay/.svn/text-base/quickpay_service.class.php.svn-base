 ...
 *
 */
require_once(dirname(__FILE__) . '/quickpay_conf.class.php');
...
class quickpay_service
{
   ...
   /**
	 ...
	 * 4位UID尾号
     */
    static function make_seed($uid,$pay_id='ERR',$pay_mode=5,$trade_num=1)
    {   
        return $pay_mode.sprintf('%02s', $trade_num).sprintf('%010s', $pay_id).rand(100,999).sprintf("%04s",substr($uid,-4));
    }
	
	static function get_seed($trade_number)
    {   
        return intval(substr($trade_number,3,10));
    }
    ...
    static function sign($params, $sign_method)
    {
        if (strtolower($sign_method) == "md5") {
		    ...
		    $security_key=C("payment")['unionpay']['key'];
